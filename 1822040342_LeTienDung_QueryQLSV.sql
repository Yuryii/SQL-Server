...
CREATE PROC B5
(@TENGV VARCHAR(30), @SO_DTHD INT OUTPUT, @SO_DTPB INT OUTPUT)
AS
BEGIN
	SELECT @SO_DTHD = COUNT(*) FROM GV_HDDT AS HD, GIAOVIEN AS GV WHERE HD.MSGV = GV.MSGV AND GV.TENGV = @TENGV
	SELECT @SO_DTPB = COUNT(*) FROM GV_PBDT AS PB, GIAOVIEN AS GV WHERE PB.MSGV = GV.MSGV AND GV.TENGV = @TENGV
END
GO
DECLARE @SO_DTHD INT
DECLARE @SO_DTPB INT
EXEC B5 'Nguyễn Văn A', @SO_DTHD OUTPUT, @SO_DTPB OUTPUT
PRINT 'SO DE TAI HUONG DAN LA: ' + CAST(@SO_DTHD AS VARCHAR) + ' VA SO DE TAI PHAN BIEN LA: ' + CAST(@SO_DTPB AS VARCHAR)

-- 6 Đưa vào TenHH ( Tên học hàm) trả ra số giáo viên thỏa học hàm đó nếu không thấy trả ra 0
CREATE PROC B6
(@TENHH VARCHAR(20), @SO_GV INT OUTPUT)
AS
BEGIN
	SELECT @SO_GV = COUNT(*) FROM GIAOVIEN AS GV INNER JOIN HOCHAM AS HH ON GV.MSHHAM = HH.MSHH WHERE HH.TENHH = @TENHH
END
GO
DECLARE @SO_GV INT
EXEC B6 'Phó giáo sư', @SO_GV OUTPUT
PRINT 'SO GIAO VIEN THOA HOC HAM DO LA: '+ CAST(@SO_GV AS VARCHAR)

--7 Đưa vào MSDT ( Mã số đề tài) trả ra số sinh viên tham gia đề tài đó nếu không Thấy trả ra 0
CREATE PROC B7
(@MSDT CHAR(6), @SO_SV INT OUTPUT)
AS
BEGIN
	SELECT @SO_SV = COUNT(*) FROM SV_DETAI AS SVDT WHERE SVDT.MSDT = @MSDT
END
GO
DECLARE @SO_SV INT
EXEC B7 '97001', @SO_SV OUTPUT
PRINT 'SO SINH VIEN THAM GIA DE TAI DO LA: ' + CAST(@SO_SV AS VARCHAR)

--8 Đưa vào MSHD ( Mã số hội đồng) TRả ra số đề tài tham gia hội đồng đó, nếu không thấy trả về 0
CREATE PROC B8
(
	@MSHD SMALLINT, @SLDT INT OUTPUT
)
AS
BEGIN
	IF NOT EXISTS (SELECT MSHD FROM HOIDONG_DT WHERE @MSHD = MSHD)
	BEGIN
		SET @SLDT = 0
	END 
	ELSE 
	BEGIN
		SELECT @SLDT = COUNT(MSDT) FROM HOIDONG_DT WHERE MSHD = @MSHD
	END	
END
GO
DECLARE @SODT INT
EXECUTE B8 3, @SODT OUTPUT
PRINT 'SO LUONG DE TAI LA' + CAST(@SODT AS VARCHAR)
--9: Đưa vào MSGV ( Mã số giáo viên) trả ra số đề tài gv đó hướng dẫn
--10: Đưa vào MSGV hãy trả ra số đề tài gv đó phản biện
-- 11: Đưa vào mshd cho biết tên hội đồng quyết định được bảo vệ thật hay không
ALTER PROC SP_CAU11B_QLSV
(
	@MSHD SMALLINT, @QUYETDINH CHAR(6) OUTPUT
)
AS 
BEGIN
	SELECT @QUYETDINH = QUYETDINH FROM HOIDONG_DT WHERE @MSHD = MSHD
END
GO
DECLARE @KETQUA CHAR(6)
EXECUTE SP_CAU11B_QLSV '2', @KETQUA OUTPUT
PRINT @KETQUA
-- 12 Ứng với mỗi hội đồng bảo vệ thật cho biết: Tên đề tài, điểm cuối cùng của đề tài (điểm trung bình)
ALTER PROC SP_CAU12B_QLSV
(
	@MSHD SMALLINT, @TENDT CHAR(6) OUTPUT, @DIEMTRUNGBINH INT OUTPUT
)
AS 
BEGIN
	DECLARE @MSDT CHAR(6) 
	SELECT @MSDT = MSDT FROM HOIDONG_DT WHERE @MSHD = MSHD AND QUYETDINH = 'được'
	SELECT @TENDT = TENDT FROM DETAI WHERE (MSDT = @MSDT)

	DECLARE @DIEM_HD FLOAT
	DECLARE @DIEM_PB FLOAT
	DECLARE @DIEM_UV FLOAT
	DECLARE @SO_GVHD INT, @SO_GVPB INT, @SO_UY_VIEN INT
	DECLARE @DTB FLOAT
	SELECT @DIEM_HD = SUM(DIEM), @SO_GVHD = COUNT(MSDT) FROM GV_HDDTN WHERE MSDT = @MSDT
...
END
GO
DECLARE @TENDT CHAR(6)
DECLARE @DTB CHAR(6)
EXECUTE SP_CAU12B_QLSV '1', @TENDT OUTPUT, @DTB OUTPUT
PRINT @TENDT + @DTB
